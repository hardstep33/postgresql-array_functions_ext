...
-- array_exists: check if an element exists in any type of array (exact match)
CREATE OR REPLACE FUNCTION array_exists(anyarray, anyelement) RETURNS boolean
AS 'MODULE_PATHNAME', 'array_exists'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

-- array_exists_epsilon: check if an element exists in float array (epsilon-safe)
CREATE OR REPLACE FUNCTION array_exists_epsilon(anyarray, anyelement) RETURNS boolean
AS 'MODULE_PATHNAME', 'array_exists_epsilon'
LANGUAGE C IMMUTABLE PARALLEL SAFE;
...
BEGIN
    ...
    -- array_exists_epsilon: float8 exact match
    IF NOT array_exists_epsilon(ARRAY[0.1,0.2,0.3]::float8[], 0.2::float8) THEN
        RAISE EXCEPTION 'array_exists_epsilon float8 exact test failed';
    END IF;

    -- array_exists_epsilon: float8 epsilon match (0.1+0.2 = 0.3)
    IF NOT array_exists_epsilon(ARRAY[0.1 + 0.2, 0.5]::float8[], 0.3::float8) THEN
        RAISE EXCEPTION 'array_exists_epsilon float8 epsilon test failed';
    END IF;
